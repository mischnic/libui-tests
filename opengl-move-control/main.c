...
static void move(uiButton *b, void *d){
	static uiWindow *win2;
	uiOpenGLArea *a = (uiOpenGLArea*) d;

	if(win2 == NULL){
		uiBoxDelete(box, 2);
		win2 = uiNewWindow("Second window", 400, 300, 1);
		uiWindowSetChild(win2, uiControl(a));
		uiControlShow(uiControl(win2));
	} else {
		uiWindowSetChild(win2, NULL);
		uiControlHide(uiControl(win2));
		uiBoxAppend(box, uiControl(a), 1);
		uiControlDestroy(uiControl(win2));
		win2 = NULL;
	}
}
...
int main(void)
{
	...
	uiBoxAppend(box, uiControl(uiNewLabel("Press keys 0-9 to set scale")), 0);

	uiOpenGLArea *glarea = uiNewOpenGLArea(&AREA_HANDLER, attribs);
	uiButton *btn = uiNewButton("Move to new window (and back)");
	uiButtonOnClicked(btn, move, glarea);
	uiBoxAppend(box, uiControl(btn), 0);

	uiBoxAppend(box, uiControl(glarea), 1);
